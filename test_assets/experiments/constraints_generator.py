...
def generate_constraint_file(sim_params_path, out_file):
    with open(sim_params_path) as json_file:
        calendars_map = parse_resource_calendars(
            json.load(json_file)["resource_calendars"]
        )
        ...
        for c_id in calendars_map:
            ...
            for w_day in c_info.work_rest_count:
                ...
                for c_interv in c_info.work_intervals[w_day]:
                    min_date = (
                        c_interv.start
                        if min_date is None
                        else min(min_date, c_interv.start)
                    )
            ...
            for w_day in c_info.work_rest_count:
                ...
                daily_start_times[c_day] = (
                    min_date_str if c_info.work_rest_count[w_day][0] > 0 else None
                )
                never_work_masks[c_day] = (
                    mask_24_hour if daily_start_times[c_day] is None else 0
                )
                always_work_masks[c_day] = (
                    0  # will keep untouched to always allowing removal
                )
